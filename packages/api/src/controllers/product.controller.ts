...
export class ProductController {
  ...
  static async create(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (characteristics) {
        parsedCharacteristics = typeof characteristics === 'string' ? JSON.parse(characteristics) : characteristics;
      }
    ...
    try {
      ...
      const updatedProduct = await prisma.product.update({
        ...
        data: {
          ...
          ...(parsedCharacteristics !== undefined && { characteristics: parsedCharacteristics }),
  ...
  static async uploadImage(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (product.images.length >= 4) {
        res.status(400).json({ success: false, message: 'Maximum of 4 additional images allowed' });
        return;
      }
