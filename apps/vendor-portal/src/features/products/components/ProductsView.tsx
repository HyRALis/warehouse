...
const ProductsResults = ({ products, page, pageSize, onPageChange }: { products: ReturnType<typeof useProducts>; page: number; pageSize: number; onPageChange: (page: number) => void }) => {
    if (products.isPending) {
        return <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{Array.from({ length: 8 }, (_, index) => <Skeleton key={index} className="aspect-[4/5]" />)}</div>;
    }

    const items = products.data?.data ?? [];
    if (items.length === 0) {
        return <EmptyState icon={<PackageX className="h-12 w-12" />} title="No products found" description="Adjust your filters or add a new product." action={<Link href="/dashboard/products/new" className={buttonVariants()}><Plus className="mr-2 h-4 w-4" /> Add Product</Link>} />;
    }

    return <>
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{items.map((product) => <ProductCard key={product.id} product={product} />)}</div>
        {products.data && <Pagination page={page} totalPages={products.data.meta.totalPages} totalItems={products.data.meta.total} pageSize={pageSize} onPageChange={onPageChange} />}
    </>;
};
...
export const ProductsView = () => {
    ...
    return <div className="mx-auto max-w-7xl space-y-6">
        ...
        <ProductsResults products={products} page={filters.page} pageSize={12} onPageChange={(page) => void setFilters({ page })} />
