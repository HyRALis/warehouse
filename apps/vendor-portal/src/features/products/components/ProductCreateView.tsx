...
import { createProductRequestSchema, type ProductStatus } from '@inventory-system/contracts';
...
const productFormSchema = z.object({
    ...
    sku: createProductRequestSchema.shape.sku,
    barcode: z.string(),
    categoryId: createProductRequestSchema.shape.categoryId,
    status: createProductRequestSchema.shape.status.unwrap(),
...
export const ProductCreateView = () => {
    ...
    const form = useAppForm({
        ...
        onSubmit: async ({ value }) => {
            setFormError(null);
            const parsed = createProductRequestSchema.safeParse({ ...value, barcode: value.barcode || undefined, characteristics: value.characteristics.filter((item) => item.name || item.value) });
