...
export default function SearchableCategorySelect({
    ...
    const selectable = useMemo<SelectableCategory[]>(() => {
        ...
        const parentIds = new Set(
            allCategories.map((category) => category.parentId).filter(Boolean)
        );
        return allCategories
            ...
            .map((category) => {
                ...
                const rootName =
                    parent?.name || (category.vendorProfileId ? 'Your categories' : 'Other');
    ...
    const filtered = useMemo(() => {
        ...
        return normalized
            ? selectable.filter((category) => category.searchableText.includes(normalized))
            : selectable;
    }, [query, selectable]);
    ...
    const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
            ...
            setActiveIndex(
                (index) => (index + direction + filtered.length) % Math.max(filtered.length, 1)
            );
        }
    ...
    const groups = useMemo(() => {
        ...
        filtered.forEach((category) =>
            result.set(category.rootName, [...(result.get(category.rootName) || []), category])
        );
    ...
    return (
        <div ref={rootRef} className="relative">
            <div
                className={cn(
                    'flex items-center rounded-lg border bg-slate-950 transition focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-500/20',
                    open ? 'border-indigo-500' : 'border-slate-800',
                    disabled && 'opacity-60'
                )}
            >
                ...
                <button
                    ...
                    className="mr-2 rounded p-1 text-slate-500 hover:text-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
                >
                    <ChevronDown
            ...
            {open && (
                <div
                    ...
                    className="absolute z-40 mt-2 max-h-80 w-full overflow-y-auto rounded-xl border border-slate-700 bg-slate-900 p-2 shadow-2xl shadow-black/40"
                >
                    {groups.length === 0 ? (
                                ...
                                {items.map((category) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={cn(
                                                'flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm outline-none',
                                                index === activeIndex
