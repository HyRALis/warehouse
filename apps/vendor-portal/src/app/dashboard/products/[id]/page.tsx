...
import ProductVersionManager, {
    ManagedProductVersion,
} from '@/components/ProductVersionManager';
...
export default function ProductDetailPage() {
    ...
    const deleteProduct = async () => {
        ...
        try {
            ...
            setError(deleteError?.message || 'Could not delete this product');
            setIsDeleting(false);
        }
    };

    if (loading) {
        return <div className="flex h-64 items-center justify-center"><Spinner size={8} /></div>;
    }
    ...
    if (error || !product) {
        return (
            <div className="mx-auto max-w-3xl rounded-xl border border-rose-500/20 bg-rose-500/10 p-6 text-center">
                ...
                <p className="mb-6 text-rose-200">{error || 'Product not found'}</p>
    ...
    return (
        <div className="mx-auto max-w-6xl space-y-7">
            <div className="flex items-center gap-2 text-sm text-slate-400">
                <Link href="/dashboard/products" className="flex items-center gap-1 hover:text-white">
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                            <Badge variant={badgeVariant(product.status)}>Product: {product.status}</Badge>
                            <Badge variant="outline">{product.versionCount || product.versions.length} versions</Badge>
                            {product.category && <Badge>{product.category.name}</Badge>}
                        </div>
                    </div>
                </div>
                <Button variant="destructive" disabled={isDeleting} onClick={() => void deleteProduct()}>
